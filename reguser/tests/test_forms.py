...
class AcceptTermsUserCreationFormTestCase(TestCase):
    ...
    def test_form_is_valid_for_valid_input_with_whitelist_subdomain(self):
        form = AcceptTermsUserCreationForm(data=self.get_form_data(email="jojo@find.me.com"), ALLOWED_EMAIL_DOMAINS=['.me.com', '.other.com'])
        self.assertTrue(form.is_valid())
    
    def test_form_is_valid_for_valid_input_with_whitelist_hidden_domain(self):
        form = AcceptTermsUserCreationForm(data=self.get_form_data(email="jojo@hide.com"), ALLOWED_EMAIL_DOMAINS=['hide.com!', 'me.com'])
        self.assertTrue(form.is_valid())

    def test_form_is_valid_for_valid_input_with_whitelist_hidden_subdomain(self):
        form = AcceptTermsUserCreationForm(data=self.get_form_data(email="jojo@i.hide.com"), ALLOWED_EMAIL_DOMAINS=['.hide.com!', 'me.com'])
        self.assertTrue(form.is_valid())
    ...
    def test_form_validation_fails_for_email_not_in_whitelist(self):
        form = AcceptTermsUserCreationForm(data=self.get_form_data(email='test@me.com'), ALLOWED_EMAIL_DOMAINS=['allowed.com', 'allowed2.com'])
        self.assertFalse(form.is_valid())
        self.assertIn(u"Only e-mail addresses from the following domains are allowed: allowed.com, allowed2.com", form.errors['email'])
    
    def test_hidden_whitelist_domain_not_shown_in_error_message(self):
        form = AcceptTermsUserCreationForm(data=self.get_form_data(email='test@not.allowed.com'), ALLOWED_EMAIL_DOMAINS=['allowed.com', 'hidden.com!'])
        self.assertFalse(form.is_valid())
        self.assertIn(u"Only e-mail addresses from the following domains are allowed: allowed.com", form.errors['email'])
    
    def test_form_validation_fails_for_email_not_in_whitelist_2(self):
        form = AcceptTermsUserCreationForm(data=self.get_form_data(email='test@me.com'), ALLOWED_EMAIL_DOMAINS=['.allowed.com'])
        self.assertFalse(form.is_valid())
        self.assertIn(u"Only e-mail addresses from the following domains are allowed: .allowed.com", form.errors['email'])

    def test_form_validation_fails_for_email_subdomain_not_in_whitelist(self):
        form = AcceptTermsUserCreationForm(data=self.get_form_data(email='test@me.allowed.com'), ALLOWED_EMAIL_DOMAINS=['allowed.com'])
        self.assertFalse(form.is_valid())
        self.assertIn(u"Only e-mail addresses from the following domains are allowed: allowed.com", form.errors['email'])

    def test_form_validation_fails_for_duplicate_email(self):
        self.USER_MODEL.objects.create_user('test', 'existing@me.com', 'passwd')
        form = AcceptTermsUserCreationForm(data=self.get_form_data(email='existing@me.com'))
        self.assertFalse(form.is_valid())
        self.assertIn(u"A user with this e-mail address already exists.", form.errors['email'])
